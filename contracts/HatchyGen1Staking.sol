    ...
    OwnableUpgradeable
{
    ...
    {
        uint256 tokenCount = userBlanaces[_account].length();
        ...
        whenNotPaused
    {
        for (uint256 i = 0; i < _stakeParams.length; i++) {
            ...
            if (signerCheck) {
                ...
                require(tokenId < MAX_HATCHIES, "Invaild token id");
    ...
    function stake(MetaInfo calldata _param, address referrer)
        public
        nonReentrant
        whenNotPaused
    {
        ...
        require(
            IERC721(hatchyNft).isApprovedForAll(_msgSender(), address(this)),
            "Not approve nft to staker address"
        );
        ...
        emit Staked(_msgSender(), tokenId, newWeight);
    }
